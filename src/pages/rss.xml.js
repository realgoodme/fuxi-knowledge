...
export async function GET() {
  ...
  const items = articles.map((entry) => {
    const date = entry.data.date ?? entry.data.ingested ?? new Date("2026-01-01");
    return `<item><title>${escapeXml(entry.data.title)}</title><link>${site}${base}/articles/${entry.id}/</link><guid>${site}${base}/articles/${entry.id}/</guid><pubDate>${date.toUTCString()}</pubDate><description>${escapeXml(entry.data.summary ?? "")}</description></item>`;
  }).join("");
